...
function validarIdentificacion($strCedulaR, $strPasaporte){

	if($strCedulaR!=null or $strPasaporte!=null){
		return true;
...
if(validarIdentificacion($_POST['cedulaVendedor'],$_POST['pasaporteVendedor'])==true){
	if($_POST['cedulaVendedor']!=null){
		$validarCedula =validarCI($_POST['cedulaVendedor']);
		$pasaporteVendedor=$_POST['pasaporteVendedor'];
		$pass=$_POST['cedulaVendedor'];
	...
	}else if($_POST['pasaporteVendedor']!=null){
		$pasaporteVendedor=$_POST['pasaporteVendedor'];
		$cedulaVendedor=$_POST['cedulaVendedor'];
		$pass=$_POST['pasaporteVendedor'];
	}
...
if($contador==0){
	...
	$row = $id->fetch_assoc();
	$idCod=$row['MAX(ID_VEN)']+1;

	$codVendedor="VEN".$idCod;
	$password=$pass.$codVendedor;
	$passwd=hash('sha512', $password);


	$consulta=$conexion->query("INSERT into vendedor (NOMBRE1_VEN,NOMBRE2_VEN,APELLIDO1_VEN,APELLIDO2_VEN,CEDULA_VEN,PASAPORTE_VEN,TLF_VEN,PASS_VEN,COD_VEN) values('$nombre1','$nombre2','$apellido1','$apellido2','$cedulaVendedor','$pasaporteVendedor','$telefonoVendedor','$passwd','$codVendedor');") or die ("no sirve");
	$id=$conexion->query("select MAX(ID_VEN) from vendedor ");
	$conexion=null;
	echo "todo bien ";
